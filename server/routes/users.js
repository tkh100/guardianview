...
router.get('/', ...requireRole('admin'), (req, res) => {
  const users = db.prepare(
    'SELECT id, username, role, cabin_group, created_at FROM app_users ORDER BY role, username'
  ).all();
  res.json(users);
});
...
router.post('/', ...requireRole('admin'), async (req, res) => {
  ...
  const result = db.prepare(
    'INSERT INTO app_users (username, password_hash, role, cabin_group) VALUES (?,?,?,?)'
  ).run(username, hash, role, cabin_group || null);

  const user = db.prepare('SELECT id, username, role, cabin_group, created_at FROM app_users WHERE id=?').get(result.lastInsertRowid);
